...
#define CLRC630_PRINTF(...) CLRC630_DEBUG_PRINTF(__VA_ARGS__)
#else
#define CLRC630_PRINTF(...)
#endif
...
/*!
  ...
  The FIFO is located at register `#CLRC630_REG_FIFODATA`. 
...
/*!
  ...
  \warning This reads regardless of `#CLRC630_REG_FIFOLENGTH`, if there aren't enough bytes present in the FIFO, they
...
/*!
  ...
  protocol is `#CLRC630_PROTO_ISO14443A_106_MILLER_MANCHESTER` which is the default protocol for the SELECT procedure.

  The most common protocols are listed in the datasheet, but the CLRC630 Quickstart Guide AN11022 gives a complete
  description.
...
/*!
  ...
  This function loads the data from the `data` array into the FIFO, and then issues the `CLRC630_CMD_TRANSCEIVE`
...
/*!
  ...
  can only be written. The key buffer is a part of memory in the CLRC630 used for the MIFARE authentication procedure.
...
/*!
  ...
  \param [in] key_type The MIFARE key A or B (`CLRC630_MF_AUTH_KEY_A` = 0x60 or `CLRC630_MF_AUTH_KEY_B` = 0x61) to use.
...
/*!
  ...
  Resets the interrupt 0 register (`CLRC630_REG_IRQ0`).
 */
void clrc663_clear_irq0();
/*!
  @brief Clear the interrupt1 flags.

  Resets the interrupt 1 register (`CLRC630_REG_IRQ1`).
 */
...
/*!
  ...
  Prints the bytes in `data` in hexadecimal format, separated by spaces using the `CLRC630_PRINTF` macro, if defined.
...
/*! \defgroup timer Timer
    ...
    The CLRC630 has 5 timers, the first four can be treated more or less similarly, the last timer `Timer4` has a
...
/*!
  ...
  This sets the the `CLRC630_REG_TCONTROL` register to enable or disable this timer.
...
/*!
  ...
  \see CLRC630_TCONTROL_STOPRX
  \see CLRC630_TCONTROL_START_NOT, CLRC630_TCONTROL_START_TX_END, CLRC630_TCONTROL_START_LFO_WO,
...
/*! \defgroup documentation From application notes
    ...
    Two application notes are of particular interest:
      - Application Note 11145:
          CLRC663, CLRC631, CLRC 630, SLRC610 Low Power Card Detection
...
/*! \brief Set the registers to the recommended values.
    ...
    `clrc663_cmd_load_protocol()` function.

    \param [in] protocol The protocol index to set the registers to. Only
           `CLRC630_PROTO_ISO14443A_106_MILLER_MANCHESTER`, `CLRC630_PROTO_ISO14443A_212_MILLER_BPSK`,
...
/*! \defgroup iso14443a ISO14443A
    ...
    terminology from the CLRC630 datasheet, which simply refers to 'card' and 'reader'.
...
/*! \brief Perform a MIFARE authentication procedure.
    ...
    internally it calls `clrc663_cmd_auth()` which is described above, the result of the
    ...
    the `clrc663_MF_deauth()` function or otherwires disabling the Crypto1 ON bit
    in the status register.

    \param [in] key_type The MIFARE key A or B ( `#CLRC630_MF_AUTH_KEY_A` = 0x60 or `#CLRC630_MF_AUTH_KEY_B` = 0x61) to
                use.
...
/*! \brief An example to read the first four blocks.
  ...
  Then attempt to authenticate block 0 using this FFkey as KEY_A type:
    \code
      clrc663_MF_auth(uid, CLRC630_MF_AUTH_KEY_A, 0)
    \endcode

  Finally, if that succeeds, we may use:
    \code
      len = clrc663_MF_read_block(0, readbuf)
    \endcode
